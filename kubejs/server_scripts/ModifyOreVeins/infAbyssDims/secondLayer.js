const InfAbyssSecondLayer = (event) => {
    ...
    vein.layeredVeinGenerator((generator) =>
      generator.buildLayerPattern((pattern) =>
        pattern
          ...
          .layer((l) =>
            l.weight(2).mat(GTMaterials.BasalticMineralSand).size(1, 1),
          ),
      ),
    );
  });
  ...
  //#endregion
  //#region deourum
  event.add("kubejs:deorum_inf2", (vein) => {
    vein.weight(12);
    vein.clusterSize(16);
    vein.density(0.8);
    vein.discardChanceOnAirExposure(1);
    ...
    vein.layeredVeinGenerator((generator) =>
      generator.buildLayerPattern((pattern) =>
        pattern
          ...
          .layer((l) => l.weight(2).mat(GTMaterials.get("deorum")).size(2, 2))
          .layer((l) => l.weight(1).mat(GTMaterials.get("goldite")).size(1, 2)),
      ),
    );
  });
};
