#include "/lib/shaderSettings/enhancedCelestials.glsl"
    ...
    #define INCLUDE_SKY_COLORS
        ...
        vec3 noonMiddleSkyColor = pow(skyColorM, vec3(1.5)) * (vec3(1.35) + rainFactorM * (ndscWeatherM + ndscRainM + ndscSnowM + ndscDryM + sandstormM))
                                + noonUpSkyColor * 0.65;
        ...
        vec3 nightColFactor      = lunarEventSkyFactor * 0.9 * vec3(0.07, 0.14, 0.24) * (1.0 - 0.5 * rainFactorM) + skyColor + 0.4 * rainFactorM * sandstormM;
