...
void DoLighting(inout vec4 color, inout vec3 shadowMult, vec3 playerPos, vec3 viewPos, float lViewPos, vec3 geoNormal, vec3 normalM, float dither,
    ...
    vec3 blockLighting = lightmapXM * blocklightCol;
        ...
        float heldLight = heldBlockLightValue; float heldLight2 = heldBlockLightValue2;
            ...
            if (heldItemId == 46148) heldLight = 7; if (heldItemId == 46148) heldLight2 = 7; // ectoplasm_bucket
            if (heldItemId == 46180) heldLight = 10; if (heldItemId == 46180) heldLight2 = 10; // lumisene_bucket
            ...
            if (heldItemId == 46148) { heldLightCol = vec4(soulFireSpecialColor.rgb * 4.0, 0.10).rgb; heldLight = 7; };  // ectoplasm_bucket
            if (heldItemId2 == 46148) { heldLightCol2 = vec4(soulFireSpecialColor.rgb * 4.0, 0.10).rgb; heldLight2 = 7; };
            if (heldItemId == 46180) { heldLightCol = vec4(7.40, 3.50, 2.60, 0.0).rgb; heldLight = 10; };  // lumisene_bucket
            if (heldItemId2 == 46180) { heldLightCol2 = vec4(7.40, 3.50, 2.60, 0.0).rgb; heldLight2 = 10; };
