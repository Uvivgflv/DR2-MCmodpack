...
float GetCloudNoise(vec3 tracePos, int cloudAltitude, float lTracePosXZ, float cloudPlayerPosY) {
    ...
    for (int i = 0; i < sampleCount; i++) {
        #if CLOUD_QUALITY_INTERNAL >= 2
            noise += Noise3D(tracePosM + vec3(wind, 0.0, 0.0) + pow(1 + i, CIRRUS_DISTORTION_INTENSITY_2) * totalDistortion) * currentPersist;
        #else
            noise += texture2DLod(noisetex, tracePosM.xz + vec2(wind, 0.0) + pow(1 + i, CIRRUS_DISTORTION_INTENSITY_2) * totalDistortion.xz, 0.0).b * currentPersist;
        #endif
