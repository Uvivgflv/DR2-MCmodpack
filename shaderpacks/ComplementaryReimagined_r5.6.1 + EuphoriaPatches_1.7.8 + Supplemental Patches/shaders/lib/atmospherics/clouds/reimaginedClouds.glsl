...
bool GetCloudNoise(vec3 tracePos, inout vec3 tracePosM, int cloudAltitude) {
    ...
    #ifdef DEFERRED1
        float noise = mix(texture2D(colortex3, coord).b, texture2D(spiral_clouds, coord).b, inMagicBiome);
    #else
        float noise = mix(texture2D(gaux4, coord).b, texture2D(spiral_clouds, coord).b, inMagicBiome);
    #endif
    ...
    for (int i = 0; i < sampleCount; i++) {
        ...
        if (GetCloudNoise(tracePos, tracePosM, cloudAltitude)) {
            ...
            float VdotSM1 = max0(sunVisibility > 0.5 ? VdotS : - VdotS);
                ...
                #ifdef DEFERRED1
                    light -= mix(texture2D(colortex3, GetRoundedCloudCoord(cLightPos.xz, CLOUD_ROUNDNESS)).b, texture2D(spiral_clouds, GetRoundedCloudCoord(cLightPos.xz, CLOUD_ROUNDNESS)).b, inMagicBiome) * cloudShadingM;
                #else
                    light -= mix(texture2D(gaux4, GetRoundedCloudCoord(cLightPos.xz, CLOUD_ROUNDNESS)).b, texture2D(spiral_clouds, GetRoundedCloudCoord(cLightPos.xz, CLOUD_ROUNDNESS)).b, inMagicBiome) * cloudShadingM;
                #endif
